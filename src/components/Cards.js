...
function Modal(props) {
	...
	const onFileUpload = async (e) => {
		...
		await axios
			.post('http://localhost:5000/api/upload', fileForm)
			.then((res) => {
				console.log('success');
	...
	const detectHandler = async () => {
		if (id === 'detectLabels') {
			await axios
				.post('http://localhost:5000/api/labels', { name: imageName })
				.then((res) => {
			...
			await axios
				.post('http://localhost:5000/api/texts', { name: imageName })
				.then((res) => {
			...
			await axios
				.post('http://localhost:5000/api/faces', { name: imageName })
				.then((res) => {
			...
			await axios
				.post('http://localhost:5000/api/celeb', { name: imageName })
				.then((res) => {
					//draw boudiry for image
					{
						let imageContainer = document.getElementById(
